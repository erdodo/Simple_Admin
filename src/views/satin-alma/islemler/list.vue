<template>
  <div>
      ...
      <el-table-column fixed="right" label="Operations" min-width="118">
        <template #default="scope">
          <div class="d-flex justify-content-center w-100">
            <el-button type="" class="me-1" @click="(visible = true), (odeme_token = scope.row.token)">Form</el-button>
            <el-dropdown trigger="click">
              <el-button><i class="bi bi-pencil fs-6"></i></el-button>
              <template #dropdown>
                <el-dropdown-menu>
                  <el-dropdown-item>
                    <router-link class="text-decoration-none text-primary" :to="'/market-islemler/detail/' + scope.row.id"
                      >Detail</router-link
                    >
                  </el-dropdown-item>
    ...
    <odeme-sorgula v-model:visible="visible" :id="odeme_token"></odeme-sorgula>
  </div>
</template>
...
import OdemeSorgula from "./odeme-sorgula.vue";
...
export default {
  ...
  methods: {
    getData() {
      services.list("market_islemler").then((res) => {
        this.datas = res.data;
      });
    },
    deleteEvent(id) {
      ElMessageBox.alert("This data is being deleted.", "Deleting", {
        ...
        callback: (action) => {
          if (action == "confirm") {
            services.del("market_islemler", id).then(() => {
              ElNotification({
                title: "Success",
                message: "Deleted",
                type: "success",
